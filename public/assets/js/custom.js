$(document).ready(function() {
  ...
  $('#profileEmail').bind('change keyup', function() {
    ...
    var uId = $('[name="id"]').val();
    $.ajax({
      url:  $('body').attr('data-base-url') + 'index.php/' + 'user/checEmailExist',
      method:'post',
      data:{
        email :email,
        uId : uId
  ...
  $('.modal-body').on('click', '.send-btn', function() {
    ...
    if($emails != ''){
      $.ajax({
        url: $('body').attr('data-base-url') + 'index.php/' + 'user/InvitePeople',
        method:'post',
        data: {
          emails: $emails
        },
  ...
  $(".content-wrapper").on("click",".modalButtonUploadImage", function(e) {
    $.ajax({
      url : $('body').attr('data-base-url') + 'user/get_modal',
      method: 'post', 
      data : {
        staff_id: $(this).attr('data-src')
      }
    }).done(function(data) {
      $('#nameModal_Photo').find('.modal-body').html(data);
      $('#nameModal_Photo').modal('show'); 
    })
  });

  $(".content-wrapper").on("click",".modalButtonEducation", function(e) {
    $.ajax({
      url : $('body').attr('data-base-url') + 'education/get_modal',
      method: 'post', 
      data : {
        edu_id: $(this).attr('data-src')
      }
    }).done(function(data) {
      $('#nameModal_Education').find('.modal-body').html(data);
      $('#nameModal_Education').modal('show'); 
    })
  });
  

  $(".content-wrapper").on("click",".modalDeleteButtonEducation", function(e) { 
    if(confirm("Are you sure to delete this record?"))
    {
      $.ajax({
        url : $('body').attr('data-base-url') + 'education/get_modal_delete',
        method: 'post', 
        data : {
          edu_id: $(this).attr('data-src')
        }
      }).done(function(data) {
        window.location.href = $('body').attr('data-base-url') + 'education';
      })
  }
  });


  $(".content-wrapper").on("click",".modalButtonFamily", function(e) {
    $.ajax({
      url : $('body').attr('data-base-url') + 'family/get_modal',
      method: 'post', 
      data : {
        f_id: $(this).attr('data-src')
      }
    }).done(function(data) {
      $('#nameModal_Family').find('.modal-body').html(data);
      $('#nameModal_Family').modal('show'); 
    })
  });

  $(".content-wrapper").on("click",".modalDeleteButtonFamily", function(e) { 
    if(confirm("Are you sure to delete this record?"))
    {
      $.ajax({
        url : $('body').attr('data-base-url') + 'family/get_modal_delete',
        method: 'post', 
        data : {
          f_id: $(this).attr('data-src')
        }
      }).done(function(data) {
        window.location.href = $('body').attr('data-base-url') + 'family';
      })
  }
  });

  $(".changeTempDistrict").on("change",function(e){      
      $.ajax({
        url : $('body').attr('data-base-url') + 'address/getdistrict/'+$(this).find(":selected").text()+'/NA',
        method: 'get',       
      }).done(function(data) {
        $('.temp_district').html(data);
      })
  });
 
  $(".changePermanentDistrict").on("change",function(e){      
    $.ajax({
      url : $('body').attr('data-base-url') + 'address/getdistrict/'+$(this).find(":selected").text()+'/NA',
      method: 'get',       
    }).done(function(data) {
...
/*  $("#nameModal_user").on("hidden.bs.modal", function(){
  ...
  /* Script for Templates Starts here */
    $('.box-body').on('click', '.view_template', function() {
      ...
      $.ajax({
        url: $('body').attr('data-base-url') + 'index.php/' + "templates/preview",
        method:'post',
        data:{
          template_id: $tmp_id
        }
  ...
  $(".content-wrapper").on("click",".templateModalButton", function(e) {  
    $.ajax({
      url : $('body').attr('data-base-url') + 'index.php/' + "templates/modal_form",
      method: "post", 
      data : {
...
function setId(id, module) {
  var url =  $('body').attr('data-base-url') + 'index.php';
